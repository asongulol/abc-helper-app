...
(() => {
  ...
  function checkElementBorders(tag, style, overrides, resolvedRadius) {
    ...
    const radius =
      resolvedRadius != null ? resolvedRadius : Number.parseFloat(style.borderRadius) || 0;
  ...
  `;
    ...
    function resolvePaintedImageRect(containerRect, image, sizeValue, positionValue) {
      ...
      if (size === 'cover' || size === 'contain') {
        ...
        if (/%$/.test(widthToken))
          paintedWidth = containerRect.width * (Number.parseFloat(widthToken) / 100);
        ...
        if (heightToken === 'auto')
          ...
          paintedHeight = containerRect.height * (Number.parseFloat(heightToken) / 100);
