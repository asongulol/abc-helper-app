...
const WISE_PAID_STATES = new Set(['outgoing_payment_sent', 'completed', 'sent']);
...
async function wiseGet<T>(path: string): Promise<T> {
  ...
  if (!r.ok) throw new Error(`Wise GET ${path} → ${r.status}: ${await r.text()}`);
...
async function handleCronReconcile(body: Record<string, unknown>): Promise<Response> {
  ...
  if (!pRes.ok) return json({ error: `payments fetch failed (${pRes.status})` }, 500);
  const payments = ((await pRes.json()) as PollPayment[]).filter((p) => p.wise_transfer_id);

  if (payments.length === 0) {
    return json({ ok: true, checked: 0, markedPaid: 0, inFlight: 0, unknown: 0 });
  }
  ...
  const outcomes = await mapLimit(payments, 8, async (p) => {
    ...
    try {
      detail = await wiseGet<Record<string, unknown>>(`/v1/transfers/${p.wise_transfer_id}`);
    } catch {
    ...
    if (WISE_PAID_STATES.has(st)) {
      ...
      return { kind: up.ok ? ('paid' as const) : ('unknown' as const), p, status: st };
    }
    if (WISE_IN_FLIGHT_STATES.has(st)) return { kind: 'inFlight' as const, p, status: st };
