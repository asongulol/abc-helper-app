...
async function getAccessToken(): Promise<string> {
	...
	if (!refresh)
		throw new Error("no Hubstaff refresh token (set HUBSTAFF_REFRESH_TOKEN)");
...
async function pageAll<T>(
	...
	for (let i = 0; i < 50; i++) {
		...
		if (!r.ok)
			throw new Error(`${key} fetch failed (${r.status}): ${await r.text()}`);
		const data = await r.json();
...
async function handleCronIngest(
	...
	if (!Number.isFinite(orgId) || orgId <= 0)
		return json({ error: "need org_id" }, 400);
	...
	const startMs =
		new Date(`${today}T00:00:00Z`).getTime() - lookback * 86_400_000;
	...
	try {
		const ptoReqs = await pageAll<{
			...
			`${API}/organizations/${orgId}/time_off_requests`,
			token,
			"time_off_requests",
		);
	...
	const links: Array<Record<string, unknown>> = wcRes.ok
		? await wcRes.json()
		: [];
	...
	for (const l of links) {
		...
		for (const src of [l.hubstaff_name as string | null, realName].filter(
			Boolean,
		) as string[]) {
	...
	const matchWorker = (uid: number, nm: string): string | null =>
		byId.get(uid) ??
		byStrict.get(nameKey(nm)) ??
		byLoose.get(looseKey(nm)) ??
		null;
	...
	const allWorkerIds = [
		...new Set(links.map((l) => String(l.worker_id ?? "")).filter(Boolean)),
	];
	...
	if (allWorkerIds.length) {
		...
		if (cRes.ok)
			for (const row of await cRes.json()) {
				...
				if (row.worker_id && !canonical.has(k))
					canonical.set(k, row.source_name);
			}
	}
	...
	if (exRes.ok)
		for (const row of await exRes.json()) {
			if (row.approval && row.approval !== "pending") {
				decidedBySrc.add(
					`${row.company_id}|${row.source_name}|${row.work_date}`,
				);
				if (row.worker_id)
					decidedByWorker.add(
						`${row.company_id}|${row.worker_id}|${row.work_date}`,
					);
			}
		}
	...
	for (const uid of userIds) {
		...
		for (const day of days) {
			...
			if (
				decidedBySrc.has(`${co}|${src}|${day}`) ||
				decidedByWorker.has(`${co}|${wId}|${day}`)
			)
				continue;
			const overall = overallDay.get(uid)?.get(day) ?? 0;
			const activityPct =
				tracked > 0 ? Math.round((overall / tracked) * 100) : null;
			rows.push({
				company_id: co,
				worker_id: wId,
				source_name: src,
	...
	if (rows.length) {
		...
		if (!up.ok)
			return json(
				{ error: `upsert failed (${up.status}): ${await up.text()}` },
				500,
			);
		written = rows.length;
	}
